...
export default function Orders() {
  ...
  const [form, setForm] = useState<"order" | "tab" | "quantity" | null>(null);
  ...
  function openForm(kind: "order" | "tab" | "quantity") { setName(""); setQuantity(kind === "quantity" ? String(active?.quantity ?? "") : ""); setTabs([""]); setFormError(""); setForm(kind); }
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (saving) return;
    setSaving(true); setFormError("");
    try {
      const { order } = await saveWorkOrder(form === "quantity" && active
        ? { action: "set-quantity", orderId: active.id, quantity: Number(quantity) }
        : form === "tab" && active
        ? { action: "add-tab", orderId: active.id, tab: name }
        : { action: "create-order", name, tabs, quantity: Number(quantity) });
  ...
  return <>
    {active ? <Board key={active.id} order={active} onBack={() => { setSelected(null); void refresh(); }} onAddTab={() => openForm("tab")} onQuantity={() => openForm("quantity")} /> : <main className="shell">
    ...
    {form && <dialog ref={dialogRef} className="order-dialog" aria-labelledby="form-title" onCancel={(event) => { event.preventDefault(); if (!saving) setForm(null); }}><form onSubmit={submit}><p className="eyebrow">{form === "order" ? "NUEVA ORDEN DE TRABAJO" : active?.name}</p><h2 id="form-title">{form === "order" ? "Crea tu OT" : form === "quantity" ? "Cantidad a fabricar" : "Agregar pestaña"}</h2>{form !== "quantity" && <label>{form === "order" ? "Nombre de la OT" : "Nombre de la pestaña"}<input autoFocus required maxLength={100} value={name} onChange={(event) => setName(event.target.value)} placeholder={form === "order" ? "OT-EP-256" : "Cocina"} disabled={saving} /></label>}
      {form !== "tab" && <label className="quantity-field">Cantidad de unidades a fabricar<input autoFocus={form === "quantity"} type="number" min="1" step="1" required value={quantity} onChange={(event) => setQuantity(event.target.value)} placeholder="Ej. 8" disabled={saving} /><small>Ejemplo: 8 barrenas requieren 8 unidades de cada tipo de ensamble. Esta meta aplica a todas las pestañas de la OT.</small></label>}
      ...
      {formError && <p role="alert" className="form-error">{formError}</p>}<div className="dialog-actions"><button className="back-button" type="button" disabled={saving} onClick={() => setForm(null)}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? "Guardando…" : form === "order" ? "Crear OT y abrir" : form === "quantity" ? "Guardar cantidad" : "Guardar pestaña"}</button></div>
    </form></dialog>}
  </>;
}
